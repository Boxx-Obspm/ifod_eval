...
[data1_distance,nb] = fscanf(f1,'%f %f %f %f %f %f %f %f %f %f %f %f', [12 inf]);
data1_distance = data1_distance';
fclose(f1);

f2=fopen(trajectory_name_ephjup,'rt');
	l=' ';
	while 1
		l=fgetl(f2);
		if strfind(l,'META_STOP')>0
...
[data1_latlong,nb] = fscanf(f2,'%f %f %f %f', [4 inf]);
data1_latlong = data1_latlong';
